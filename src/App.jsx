  ...
  return (
    <div className='bg-black text-slate-200 h-screen text-sm font-extrabold'>
      <div className='w-full flex justify-around py-2 border-b border-[#222528]'>
        <button className='bg-[#222528] text-white  w-30 rounded-2xl py-3 ' onClick={handleLogs}>Logs</button>
        <button className='bg-[#222528] text-white w-30 rounded-2xl py-3' onClick={handleActiveUsers}>Active Users</button>
      </div>
      {
        logs ? <Logs/> : <ActiveUsers/>
      }
      <div className='fixed bottom-0 flex justify-center w-full py-1'>
        <button className='bg-red-900 text-white py-3 px-6 rounded-2xl ' onClick={() => {setOpen(true)}}>delete logs</button>
      </div>
      {
